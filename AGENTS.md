...
- Data path is the CI cache, not direct fetch: `.github/workflows/update-todo.yml` fetches each source md from `raw.githubusercontent.com`, renders it, and commits `data/todo-cache.json` (`{v, generatedAt, generatedAtMs, sources:[{repo,branch,file,label,md?,error?}]}`, single-line). `v: 2` means task lines in `md` carry CI-rendered HTML: text is html-escaped first, then `$…$`/`$$…$$` → italic span via a LaTeX→Unicode map (unknown `\cmds` stay raw), backticks → monospace span, `[text](url)` → `<a>` (relative urls resolve to `github.com/{repo}/blob/{branch}/…`, so `javascript:` etc. can't pass), images stripped; task-line structure (indent, marker, `- [x] `-checkbox) is preserved verbatim so the client parse keeps working. Triggers: cron 08:00/14:00/19:00 Beijing (`0 0,6,11 * * *`), `workflow_dispatch`, push on `data/todo-sources.json`; commits only when the cache actually changed (stage first, then `git diff --cached --quiet` guard). The cache file is fork-only (upstream never has it), so the daily upstream merge stays conflict-free — but delete any local test copy: an untracked `data/todo-cache.json` blocks a later pull/merge.
